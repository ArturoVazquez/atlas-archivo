...
**Versión del contrato:** 1.69.0 · **Fecha:** 2026-08-21
...
**Y por qué NO es una historia** *(1.69)* — la puerta que a esta sección le
faltaba, porque cuando se escribió (1.44, el 2026-08-15) las Historias todavía
no existían: nacieron tres días después. Un conjunto publica **cifras**; una
historia publica **el relato** que las interpreta, y **no añade ni un dato al
atlas**. Las dos condiciones del conjunto son **acumulativas**: no basta con que
el hecho no tenga lugar, **tiene que ser un número**. Una regla de perímetro
—qué entra en una capa y qué no— o una ausencia documentada no son cifras: la
primera va en la nota `_` del manifiesto y en los `hueco` de las fichas, y la
segunda se cuenta en una historia. Ver **D19**, que es donde vive el porqué.
...
- *espacios-maritimos* *(1.11)*: `sin_delimitar` · `limite_declarado` ·
  `limite_acordado` *(1.61)* · `linea_media_sin_acuerdo` *(1.63)* ·
  `linea_cierre` *(1.63)* · `recurso_afectado`
...
>
> *(1.62)* **Las líneas de base rectas del RD 2510/1977** entran como once
> registros `MultiLineString` — uno por fachada o grupo insular, que es el
> reparto del propio anexo — y con ellas el esquema admite esa geometría: los
> tramos de una fachada son disjuntos porque entre ellos la costa sigue con su
> línea de bajamar, y partirlos en registros sueltos inventaría una estructura
> que el acto no tiene. La discrepancia de punta Grieta —impresa 32 km mar
> adentro de Alegranza, sin que la corrección de errores de 1977 ni ningún acto
> posterior la toque— se dibuja como el acto la publica y queda contada en su
> clave: con los primarios que no cuadran consigo mismos, el atlas cuenta la
> discrepancia en vez de esconderla.
>
> *(1.63)* **Un registro dibujado, una semántica.** Las siete polilíneas de
> Gibraltar del fichero del UKHO entraron como un solo registro y el mapa las
> leía como dos límites exteriores. El fichero británico distingue TRES clases
> en su campo `Feature`, y el atlas pasa a respetar ese reparto con un registro
> por clase: el mar territorial reclamado (`limite_declarado`), las líneas
> medianas «in absence of agreed maritime boundary» (**`linea_media_sin_acuerdo`**,
> familia visual del `sin_delimitar`, en trazo CONTINUO porque el discontinuo
> de esta casa significa «no verificado») y las líneas de cierre de las bocanas
> del puerto (**`linea_cierre`**, categoría técnica): una línea de cierre
> pertenece al sistema de líneas de base, no a los límites exteriores, y
> vestirla de límite la haría mentir.
>
...
| **1.67.0** | 2026-08-21 | **Aditiva, y levanta una prohibición reescribiendo su motivo (D17):** `centros-datos` recupera `potencia_it_mw`, que estaba PROHIBIDO por su nombre desde que la capa nació. El motivo decía «ningún acto administrativo la da: la publican la patronal y los propios operadores, que por §6.1 son `corporativa` y por R3 no sostienen un `confirmado`», y **dejó de ser cierto el 2026-08-14**: el Acuerdo del Gobierno de Aragón sobre el campus Zaragoza-WIND escribe «contará con una potencia total de 144 MW IT» en su propia descripción del proyecto que declara. La prohibición no se borra: **se sustituye por una vara**, escrita en el esquema, que hace el mismo trabajo sin cegar el campo — solo lo llena un ACTO que AFIRME la cifra, y no la llenan ni una nota corporativa, ni la memoria del promotor recitada por el acto («según la información facilitada por la promotora»), ni un proyecto en información pública; todo eso sigue yendo a `claves`, como en ACS, Alcalá y Navalmoral. De quince registros lo llena **uno**, y eso retrata la capa mejor que un campo que no existe: un registro sin él no está incompleto, es un centro cuya potencia TI no ha publicado nadie con autoridad. **No nace ninguna regla `R*`.** |
...
| **1.62.0** | 2026-08-19 | **Aditiva, y es el cero del mapa marítimo.** Entran las **líneas de base rectas del RD 2510/1977** — el punto desde el que se mide todo lo demás: 154 puntos en 31 tramos, en once registros `MultiLineString` cuyo reparto (tres fachadas, tres grupos baleares, cinco canarios) es el del propio anexo. El esquema de `espacios-maritimos` admite por eso la multilínea: los tramos de una fachada son disjuntos porque entre ellos la costa sigue con su bajamar. **Cómo se transcribió, porque el camino es la garantía:** la tabla HTML del BOE conserva el entrelazado a dos columnas del facsímil y traía un bloque corrido una fila y dos celdas perdidas; se recompuso con un plan de costura explícito **validado por el encadenamiento de topónimos del propio anexo** («De X» sin coordenadas debe igualar al «a X» anterior — un empalme mal hecho rompe la cadena y el guion para), las celdas se restauraron del facsímil archivado, y los 154 puntos se contrastaron contra el litoral provincial del IGN: los tres discrepantes se zanjaron leyendo el papel a 400 ppp. **Lo que el papel dice y no cuadra, se cuenta:** «Pta. Grieta (Alegranza), 29°42′,50» cae 32 km mar adentro cuando su vecina Delgada pisa la isla — huele a errata de imprenta (¿42 por 24?), pero la corrección de errores de 20-10-1977 (BOE-A-1977-25293, archivada) **solo toca el preámbulo** y ningún acto posterior modifica el anexo, así que se dibuja como está impreso y la clave lo cuenta (la misma doctrina del FOS_30). **Dos aclaraciones que la investigación dejó:** España **no ha depositado** las líneas de base ante la ONU — el M.Z.N.19.1998 que lo parecía es la zona de protección pesquera del RD 1315/1997 («geodetic system Potsdam», dice el depósito), hoy sustituido por el M.Z.N.34.2000, y la nota francesa de 1998 responde a ESO, no a las líneas de base —queda anotado como candidato de capa—; y **Ceuta y Melilla no aparecen en el anexo**, silencio que se declara en PROCEDENCIA. Pruebas **71 → 71**. **No nace ninguna regla `R*`.** |
